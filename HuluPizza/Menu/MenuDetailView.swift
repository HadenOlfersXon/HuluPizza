        ...
        VStack {
            HStack {
                VStack(alignment: .trailing) {
                    ...
                    ScrollView {
                        Text(item?.description ?? "We're a bunch of surfers who love pizza anbd Island food. Explore our favorites here in our restaurant and food trucks")
                    }
                    HStack {
            ...
            HStack{
                Picker(selection: $pizzaCrust) {
                    ForEach(PizzaCrust.allCases,id:\.self){crust in Text(crust.rawValue).tag(crust as PizzaCrust?) }
                    } label: {
                        Text("Pizza Crust")
                    }
                    .padding(.leading,50)
                ...
                VStack{
                    HStack{
                        ...
                        Button {
                            ...
                            Spacer()
                            Text((item?.price ?? 0) *
                                 Double(quantity),format:.currency(code: "USD")).font(.title).bold()
